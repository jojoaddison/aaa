...
public class ArticleResource {
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the article, or with status 404 (Not Found)
     */
    @RequestMapping(value = "/articles/by-pid/{pid}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<Article> getArticleByPid(@PathVariable String pid) {
        log.debug("REST request to get Article by PID : {}", pid);
        Article article = articleRepository.findByPid(pid);
        return Optional.ofNullable(article)
            .map(result -> new ResponseEntity<>(
                result,
                HttpStatus.OK))
